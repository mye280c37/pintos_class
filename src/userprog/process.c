...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Argument Passing - 20190258 */
  // argv[i] passing
  char **arg_addr = (char**)malloc(sizeof(char*)*argc);
  int argv_bytes = 0;
  for(i=argc-1; i>=0; i--){
    *esp -= (strlen(argv[i]) + 1);
  ...
  // word align
  //int word_align_num = ROUND_UP(argv_bytes, 4) % 4;
  int word_align_num = 4 - argv_bytes % 4;
  char *word_align = (char*)calloc(word_align_num, sizeof(char));
  *esp -= word_align_num;
  strlcpy(*esp, word_align, word_align_num);
  // arg[i] address passing
  *esp -= 4;
  *(char**)*esp = 0;
  for(i=argc-1; i>=0; i--){
    *esp -= 4;
  ...
  // argv address passing
  *esp -= 4;
  *(char***)*esp = (char**)((*esp) + 4);
  // arg passing
  *esp -=4;
  *(int*)*esp = argc;
  // return address passing : need additional study
  *esp -= 4;
  *(char**)*esp = (char*) 0;
